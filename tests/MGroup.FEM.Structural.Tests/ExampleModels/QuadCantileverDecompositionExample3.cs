...
{
	public class QuadCantileverDecompositionExample3
	{
		public static Model CreateModel()
		{
			...
			for (var i = 0; i < 5; i++)
			{
				for (var j = 0; j < 2; j++)
				{
					...
					{
						model.NodesDictionary[i * 3 + j],
						model.NodesDictionary[(i + 1) * 3 + j],
						model.NodesDictionary[(i + 1) * 3 + j + 1],
						model.NodesDictionary[i * 3 + j + 1]
					});
			...
			var loads = new[]
			{
				new NodalLoad(model.NodesDictionary[17], StructuralDof.TranslationY, amount: 1000)
			};
